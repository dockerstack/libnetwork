...
func TestKVObjectHierarchicalKeys(t *testing.T) {
	store := NewTestDataStore()
	err := store.PutObject(dummyKVObject("2000", false))
	if err != nil {
		t.Fatal(err)
	}
	keychain := []string{dummyKey, "2000"}
	_, _, err = store.KVStore().Get(Key(keychain...))
	if err != nil {
		t.Fatal(err)
	}
}
